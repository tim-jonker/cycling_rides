...
etappe_uitslag = pd.read_excel(
    f"C:\\Users\\timjo\\OneDrive\\Tim\\TourDeFrance\\Ideale Tourselectie {year}.xlsx",
    sheet_name="Top20",
)
points_per_etappe = pd.read_excel(
    f"C:\\Users\\timjo\\OneDrive\\Tim\\TourDeFrance\\Ideale Tourselectie {year}.xlsx",
    sheet_name="Points",
)
...
klassement = pd.read_excel(
    f"C:\\Users\\timjo\\OneDrive\\Tim\\TourDeFrance\\Ideale Tourselectie {year}.xlsx",
    sheet_name="Klassement",
)

etappe_uitslag.dropna(inplace=True)
klassement.dropna(inplace=True, subset=["Alg."])
klassement.fillna("Unaffordable", inplace=True)
riders = riders._append(
    {"Team": "UNA", "Name": "Unaffordable", "Cost (mln)": BUDGET}, ignore_index=True
)
...
etappe_uitslag = etappe_uitslag.merge(
    points_per_etappe[["Positie", "Points"]], on="Positie"
)
klassement = klassement.merge(
    points_per_etappe[["Positie"] + KLASSEMENTEN], on="Positie"
)

points = pd.DataFrame(
    {
        "Renner": list(riders["Name"]) * ETAPPES,
        "Etappe": np.repeat(range(1, ETAPPES + 1), len(riders)),
    }
)
points = points.merge(etappe_uitslag, how="outer", on=["Renner", "Etappe"])
for klas in KLASSEMENTEN:
    points = points.merge(
        klassement[["Etappe", klas + "_x", klas + "_y"]],
        how="outer",
        left_on=["Etappe", "Renner"],
        right_on=["Etappe", klas + "_x"],
    )
    points.drop(columns=[klas + "_x"], inplace=True)

points["Points"] = points[["Points"] + [klas + "_y" for klas in KLASSEMENTEN]].sum(
    axis=1
)
...
for etappe in range(1, ETAPPES + 1):
    winnaar = etappe_uitslag.loc[
        (etappe_uitslag["Etappe"] == etappe) & (etappe_uitslag["Positie"] == 1),
        "Renner",
    ].iat[0]
    winnende_ploeg = points_pivot.at[winnaar, "Team"]
    points_pivot.loc[points_pivot["Team"] == winnende_ploeg, etappe] += team_points[
        "Etappe"
    ]
    points_pivot.at[winnaar, etappe] -= team_points["Etappe"]

    for klas in KLASSEMENTEN:
        winnaar = klassement.loc[
            (klassement["Etappe"] == etappe) & (klassement["Positie"] == 1), klas + "_x"
        ].iat[0]
        winnende_ploeg = points_pivot.at[winnaar, "Team"]
        points_pivot.loc[points_pivot["Team"] == winnende_ploeg, etappe] += team_points[
            klas
...
select_rider = plp.LpVariable.dicts(
    "select",
    (rider for rider in points_pivot.index),
    lowBound=0,
    upBound=1,
    cat="Integer",
)
assignment = plp.LpVariable.dicts(
    "select",
    (
        (rider, etappe)
        for rider in points_pivot.index
        for etappe in range(1, ETAPPES + 1)
    ),
    lowBound=0,
    upBound=1,
...
model += plp.lpSum(
    points_pivot.at[renner, etappe] * assignment[(renner, etappe)]
...
model += (
    plp.lpSum(
        points_pivot.at[renner, "Cost (mln)"] * val
        for renner, val in select_rider.items()
    )
    <= BUDGET
)
...
for etappe in range(1, ETAPPES + 1):
    model += (
        plp.lpSum(assignment[(renner, etappe)] for renner in points_pivot.index) == 9
    )
...
for team in riders["Team"].unique():
    for etappe in range(1, ETAPPES + 1):
        model += (
            plp.lpSum(
                assignment[(renner, etappe)]
                for renner in points_pivot.index
                if team_dict.at[renner, "Team"] == team
            )
            <= 4
        )
