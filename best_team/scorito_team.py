...
from data_reader.read_data import (
    read_scorito_data,
    read_wielerorakel,
    read_gpx_file,
    save_gpx_to_disk,
...
def modify_sprint_scores(
    data: pd.DataFrame, scores: pd.DataFrame, no_mass_sprints: List[str]
) -> pd.DataFrame:
    pure_sprint_stages = value.columns[value.loc["Type", :] == "Sprint"]
    all_sprint_stages = value.columns[
        (value.loc["Type", :] == "Sprint") | (value.loc["Type", :] == "SprintHill")
    ]
    best_sprinter_per_team = list(
        data.loc[data.groupby("team")["sprint"].idxmax()].index
    )
    for rider in scores.index:
        ...
        scores.loc[rider, pure_sprint_stages] += (
            leadout_points - data.at[rider, "leadout"]
        ) / (7 * 3)
...
def add_team_points(scores: pd.DataFrame) -> pd.DataFrame:
    ...
    for stage, leader in leaders.items():
        ...
        in_winning_team = list(
            data.index[(data["team"] == winning_team) & (data.index != leader)]
        )
...
def add_etappe_pred(scores: pd.DataFrame) -> pd.DataFrame:
    et_pred = (
        pd.read_excel("Wielrennen.xlsx", sheet_name="Giro2024Et")
        .fillna(0)
        .set_index("fullName")
    )
...
def model(
    scores: pd.DataFrame,
    data: pd.DataFrame,
    hardcode_pick: List[str],
    hardcode_dont_pick: List[str],
):
    ...
    y = {
        (rider, stage): plp.LpVariable(
            f"Select {rider} in {stage}", 0, 1, plp.LpContinuous
        )
        for rider in riders
        for stage in stages
    }
    z = {
        (rider, stage): plp.LpVariable(
            f"Leader {rider} in {stage}", 0, 1, plp.LpContinuous
        )
        for rider in riders
        for stage in stages
    }
    ...
    model += plp.lpSum(
        scores.at[rider, stage] * (y[rider, stage] + z[rider, stage])
        for rider in riders
        for stage in stages
    )
    ...
    for stage in stages:
        for rider in riders:
            model.addConstraint(
                y[rider, stage] <= x[rider], f"{rider} select in {stage} if picked"
            )
            model.addConstraint(
                z[rider, stage] <= y[rider, stage],
                f"{rider} lead in {stage} if selected",
            )

    model.addConstraint(
        plp.lpSum(data.at[rider, "Prijs"] * x[rider] for rider in riders) <= BUDGET,
        "Budget constraint",
    )

    model.addConstraint(plp.lpSum(x[rider] for rider in riders) <= 20, "Max num riders")

    for stage in stages:
        model.addConstraint(
            plp.lpSum(y[rider, stage] for rider in riders) <= 9,
            f"Max 9 riders in stage {stage}",
        )
        model.addConstraint(
            plp.lpSum(z[rider, stage] for rider in riders) <= 1,
            f"Max 1 leader in stage {stage}",
        )
...
if __name__ == "__main__":
    ...
    value = (
        stage_classification.merge(required_skill, left_on="Type", right_index=True)
    ...
    selected_riders, stage_selection = model(
        scores, data, hardcode_pick, hardcode_dont_pick
    )
