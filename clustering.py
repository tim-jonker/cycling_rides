...
def get_surface(longitudes: List[float], latitudes: List[float]) -> float:
    ...
    course = LineString([(lon, lat) for lon, lat in zip(longitudes, latitudes)]).buffer(
        0.01
    )
    ...
    cobble_surfaces = {"unhewn_cobblestone", "cobblestone"}
    ...
    for way in result.ways:
        way_line = LineString(
            [transformer.transform(float(n.lon), float(n.lat)) for n in way.nodes]
        )
        intersection_length = (
            way_line.buffer(10, cap_style="flat").intersection(course).length / 1000
        )
        surface = way.tags.get("surface", "unknown")
        surface_cat = surface_category.get(surface, "unknown")
        surface_counts[surface_cat] = (
            surface_counts.get(surface_cat, 0) + intersection_length
        )

    return surface_counts.get("unpaved", 0) / (
        surface_counts.get("paved", 1) + surface_counts.get("unpaved", 0)
    )
...
def climb_count(df: pl.DataFrame, segment_length: float) -> pl.DataFrame:
    ...
    return (
        smoothed.select("course", "elevation", "distance_from_start")
        .with_columns(
            (pl.col("elevation").shift(-1).over("course") - pl.col("elevation")).alias(
                "elevation_diff"
            ),
            (
                pl.col("distance_from_start").shift(-1).over("course")
                - pl.col("distance_from_start")
            ).alias("d_diff"),
        )
        .with_columns(
        ...
        .with_columns(
            (
                pl.col("vertical_direction").shift(1).over("course")
                != pl.col("vertical_direction")
            )
            .backward_fill()
            .alias("change_direction")
        )
        .with_columns(pl.col("change_direction").cum_sum().over("course"))
        .group_by(["course", "change_direction"], maintain_order=True)
        .agg(
            pl.sum("elevation_diff").alias("meters_climbed"),
            pl.sum("d_diff").alias("horizontal_dist"),
        )
        .with_columns(
            (pl.col("meters_climbed") / (pl.col("horizontal_dist") * 10)).alias(
                "average_gradient"
            )
        )
        .filter(
        ...
        .group_by("course", maintain_order=True)
        .agg(
...
def corner_count(df: pl.DataFrame, segment_length: float) -> pl.DataFrame:
    ...
    smoothed = smooth_data(df, segment_length)
    return (
        smoothed.select("course", "longitude", "latitude", "distance_from_start")
        .with_columns(
        ...
        .with_columns(
            (
                (
                    (
                        pl.col("bearing")
                        - pl.col("bearing").shift(1).over("course")
                        + 180
                    )
                    % 360
                )
            ...
            .abs()
            .alias("bearing_change")
        )
        .with_columns((pl.col("bearing_change") > 30).alias("corner"))
        .group_by("course", maintain_order=True)
        .agg(
            (pl.sum("corner") / pl.max("distance_from_start")).alias(
                "num_corners_per_km"
            ),
            (pl.sum("bearing_change") / pl.max("distance_from_start")).alias(
...
if __name__ == "__main__":
    ...
    unclassified = unclassified.with_columns(
        pl.when(pl.col("distance") <= 60)
        .then(pl.lit(num_clusters - 1))
        .when(pl.col("f_unpaved") > 0.1)
        .then(pl.lit(num_clusters - 2))
        .otherwise(pl.lit(0))
        .alias("predicted_labels")
    )
    classified = unclassified.filter(pl.col("predicted_labels") > 0)
    unclassified = unclassified.filter(pl.col("predicted_labels") == 0).drop(
        "predicted_labels"
    )
    ...
    kmeans = KMeans(
        n_clusters=num_clusters - classified["predicted_labels"].n_unique(),
        random_state=0,
        n_init="auto",
    ).fit_predict(x)
    ...
    classified = pl.concat(
        [
            classified.select("course", "predicted_labels"),
            unclassified.select("course", "predicted_labels"),
        ]
    ).sort("course")
